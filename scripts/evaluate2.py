...
def reconstructon(loader, model, ld_kwargs, num_evals,
    ...
    """
    Reconstruct crystals from loader.
    Also returns:
    - property predictions from latent z
    - ground truth labels
    """
    ...
    for idx, batch in enumerate(loader):
        ...
        # prediction from z
        with torch.no_grad():
            pred = _raw_property_preds(model, z).detach().cpu()
            predictions.append(pred)

        # ground truth y values
        batch_data_list = batch.to_data_list()
        input_data_list += batch_data_list
        batch_y = torch.stack([d.y for d in batch_data_list])  # assuming d.y exists
...
def main(args):
    ...
    model, test_loader, cfg = load_model(
        ...
        ('opt' in args.tasks and args.start_from == 'data'), testing=False)
    ...
    if 'recon' in args.tasks:
        ...
        (frac_coords, num_atoms, atom_types, lengths, angles,
         all_frac_coords_stack, all_atom_types_stack, input_data_batch, predictions, ground_truths) = reconstructon(
        ...
        torch.save({
            ...
            'lengths': lengths,
            'angles': angles,
            'predictions': predictions,     # ← latent z からの予測値
            'ground_truths': ground_truths, # ← val/test データに元々ついていた y 値
    ...
    if 'opt' in args.tasks:
        ...
        if args.label == '':
            ...
            gen_out_name = f'eval_opt_{args.label}__bg{args.target_bg:.2f}__lr{args.lr}__grad-steps{args.num_gradient_steps}__c100{args.coef_100more}__cT{args.coef_tolerance}.pt'
